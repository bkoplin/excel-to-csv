...
const program = new Commander.Command()
  ...
  .action(async (args) => {
    if (!args.filePath) {
      const answer = await inquirer.prompt({ type: 'input', name: 'filePath', message: 'Enter the path to the Excel file', when: !args.filePath, required: true, validate: (filePath: string) => {
        if (existsSync(filePath))
          return true
    ...
    if (!args.sheetName) {
      ...
      const answer = await inquirer.prompt({ type: 'list', name: 'sheetName', message: 'Select the worksheet to parse', choices: SheetNames })
      args.sheetName = answer.sheetName
    }
    if (!args.range) {
      const wb = XLSX.readFile(args.filePath, { sheet: args.sheetName })
      const answer = await inquirer.prompt({ type: 'input', name: 'range', message: 'Enter the range of the worksheet to parse', default: wb.Sheets[args.sheetName]['!ref'] })
